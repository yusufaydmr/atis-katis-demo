...
export function HistoryTable() {
  const { shipments, vehicles, wasteTypes, companies, currentCompanyId } = useMockData();
  const activeReceiverId = currentCompanyId || "comp_receiver_1";
  const completedShipments = shipments.filter(s => s.receiverId === activeReceiverId && s.status === "DELIVERED");
  ...
  const handleDownloadExcel = () => {
    const excelData = completedShipments.map(s => {
        const vehicle = vehicles.find(v => v.id === s.vehicleId);
        return {
            "Transfer ID": s.id.toUpperCase(),
            "Kabul Tarihi": formatDate(s.createdAt),
    ...
    exportToCSV(excelData, "Gecmis_Teslimatlar");
  };
  ...
  return (
    <Card className="border shadow-md">
      <CardHeader className="flex flex-row items-center justify-between bg-gray-50/50 border-b py-4">
          <div>
              <CardTitle className="text-lg font-bold text-gray-800">Tamamlanan İşlemler</CardTitle>
              <p className="text-xs text-gray-500 mt-1">Daha önce kabul ettiğiniz atık transferleri.</p>
          </div>
        ...
        <Table className="border-collapse border-b border-gray-200">
          <TableHeader>
            <TableRow className="bg-gray-100 hover:bg-gray-100 border-b border-gray-300">
              ...
              <TableHead className="h-9 border-r border-gray-300 text-gray-700 font-bold text-xs uppercase w-[100px]">Kabul Tarihi</TableHead>
          ...
          <TableBody>
            {completedShipments.length === 0 ? (
              ...
              completedShipments.map((s) => {
                  const vehicle = vehicles.find(v => v.id === s.vehicleId);
                  return (
                  <TableRow key={s.id} className="hover:bg-gray-50 border-b border-gray-200 text-xs opacity-80 hover:opacity-100">
                      ...
                      <TableCell className="border-r border-gray-200 py-2 font-mono text-gray-700 font-bold">{getWasteCodeById(s.wasteTypeId)}</TableCell>
                      ...
                      <Badge variant="secondary" className="rounded-none px-2 py-0.5 text-[10px] font-normal w-full justify-center bg-green-100 text-green-800 border-green-200">
                          TESLİM ALINDI
                      </Badge>
                      </TableCell>
                  </TableRow>
