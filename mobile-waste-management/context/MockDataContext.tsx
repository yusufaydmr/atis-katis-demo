...
import { Role, Shipment, Vehicle, WasteType, ShipmentStatus, Company } from "../types";
import { initialShipments, initialVehicles, initialWasteTypes, initialCompanies } from "../data/mockData";
...
export const MockDataProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addShipment = (shipment: Shipment) => {
    ...
    Alert.alert("Başarılı", "Yeni atık kaydı başarıyla oluşturuldu!\nGüvenlik birimine bildirim gönderildi.");
  };
  ...
  const updateShipmentStatus = (id: string, status: ShipmentStatus) => {
    ...
    if (status === "ON_WAY") {
      Alert.alert("Bilgi", "Araç çıkışına onay verildi.\nSürücü yola çıktı.");
    } else if (status === "DELIVERED") {
      Alert.alert("Başarılı", "Teslimat tamamlandı!\nStok kayıtları güncellendi.");
    }
  };

  const addVehicle = (vehicle: Vehicle) => {
    setVehicles((prev) => [...prev, vehicle]);
    Alert.alert("Başarılı", "Yeni araç sisteme eklendi.");
  };

  const removeVehicle = (id: string) => {
    setVehicles((prev) => prev.filter((v) => v.id !== id));
    Alert.alert("Uyarı", "Araç kaydı silindi.");
  };

  const addWasteType = (waste: WasteType) => {
    setWasteTypes((prev) => [...prev, waste]);
    Alert.alert("Başarılı", "Yeni atık kodu tanımlandı.");
  };

  const removeWasteType = (id: string) => {
    setWasteTypes((prev) => prev.filter((w) => w.id !== id));
    Alert.alert("Uyarı", "Atık kodu silindi.");
  };
